...
240-mp-jellyfin is a focused macOS Apple Silicon fork of 240-MP. The goal is to keep the retro CRT-style media shell, keep Local, Retro, Tumblr, Nature, and Karaoke user-facing, and make Jellyfin the primary server-backed integration. Local owns the former Loop behavior.
...
- Confirm Local browsing, Play Now, track probing, sidecar subtitles, queue/soundtrack add, duplicate persistence, reorder/remove/clear, local playlist expansion, ordered YouTube soundtrack import/reorder/audio-only streaming, media-queue muting, Repeat Off/Queue/One, shuffle, resume, failed retention, configurable root, separate audio recovery, and saved-queue auto-launch.
