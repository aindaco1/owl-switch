...
- The soundtrack mpv IPC also observes the active playlist item and its runtime metadata. Local prefers embedded artist/title tags (including ID3 and equivalent container tags), then parses `Artist - Song`, and finally uses `SOUNDTRACK` with a cleaned title. `MpvController` renders that bounded, escaped metadata through one independent ASS text overlay on the actual video output for 15 seconds at each video start and each soundtrack change; still images never trigger it.
