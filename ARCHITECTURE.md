...
Think of OwlSwitch as a **browsing shell** that hands off to **purpose-built tools**.
...
- `Player.qml` hands decoded YouTube watch URLs to mpv through `MpvController`, using custom input bindings for channel surfing, clip skipping, filtering, special effects, and exit behavior. The first clip starts mpv; later clip and channel changes use `loadfile ... replace` over the existing IPC session.
...
- `Items.qml` owns the queue editor used both before and during playback. Search normalization, article-insensitive ordering, paging, and matching live in `KaraokeBackend`; marquee rows and confirmation UI remain shared QML components.
...
- `Player.qml` also derives one next-song preview policy from mpv's observed position and duration. The shared track card appears during the first and last 15 seconds, follows changes to the next queue entry, remains continuous when those windows overlap, and clears when there is no next entry. Pauses and seeks follow video time rather than a wall-clock timer.
...
- `AudioCrossfadePlayer` owns at most two isolated instances of the existing `MpvController`. Its named audio-only profile uses verified HTTPS with certifi from the pinned helper runtime, no redirects or yt-dlp extraction, no inherited mpv config/scripts/video windows, MP3 demuxing, and at most 16 MiB of memory buffering per player. Private IPC/config/log directories prevent interference with video playback. Only the next recording is prepared, muted and paused within 30 seconds of the current ending, with a 20-second readiness/progress deadline; five failures with increasing backoff stop audio without stopping images.
...
- The soundtrack pane accepts a pasted public or unlisted YouTube playlist URL. `LocalFilesBackend` canonicalizes the input to a YouTube playlist URL and runs the shared bounded yt-dlp/Deno job. The first valid video is persisted immediately and subsequent entries flush in batches of 16 in source order, so playback can begin before a large inventory finishes. Those entries use the same move/remove/clear queue operations as local audio and stream through the separate mpv process without downloading a cache.
...
- The soundtrack mpv IPC also observes the active playlist item and its runtime metadata. Local prefers embedded artist/title tags (including ID3 and equivalent container tags), then parses `Artist - Song`, and finally uses `SOUNDTRACK` with a cleaned title. `MpvController` owns one bounded, escaped, adaptively sized ASS track-card renderer shared with Karaoke; Local shows its larger centered artist/title layout on the actual video output for 15 seconds at each video start and each soundtrack change, while still images never trigger it.
...
`YouTubePolicy` owns accepted YouTube identities, canonical URLs, the 720p-video and audio-only format profiles, direct helper arguments, and mpv raw options. `YouTubeJob` owns one bounded asynchronous process lifecycle with timeout, cancellation, output caps, and sanitized errors. Karaoke and Local use both; Retro uses the same explicit mpv video profile. The package contains one `helper-manifest.json`, and CI calls one reusable canary script for all Karaoke sources, Local playlist expansion, audio playback, and 720p video playback.

The pinned official yt-dlp onedir archive installs as `Resources/bin/yt-dlp` plus `Resources/bin/_internal`. Release signing discovers every nested Mach-O file rather than assuming one executable. After the QML controller is interactive, a local `yt-dlp --version` job warms the runtime without network access.
