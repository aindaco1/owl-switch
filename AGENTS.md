...
- Nature sound uses Earth Garden’s anonymous Freesound catalog, accepts only independently validated CC0 preview identities, and caches metadata only. `NatureSoundtrack` owns the session; `AudioCrossfadePlayer` reuses two isolated `MpvController` instances for five-second overlaps. Keep audio independent of photo rotation/refresh, pause together, stop on exit, and preserve the slideshow on sound failure.
- Signed releases update through the shared Sparkle bridge behind `UpdateManager`. Preserve Ed25519 archive/feed verification, Developer ID signing, notarization, bundle identity and arm64 release gates. Keep the legacy DMG/checksum contract for pre-Sparkle installations.
