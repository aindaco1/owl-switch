...
- Nature sound requests anonymously use the fixed Earth Garden catalog endpoint and strictly validated `https://cdn.freesound.org/previews/` MP3 identities. Only explicit CC0 recordings with the expected license URL are accepted on fetch and cache load. Catalog and media redirects are disabled, metadata responses and process buffers are capped, and audio uses verified TLS with the certificate bundle from the checksum-pinned yt-dlp runtime and no user mpv configuration, scripts, or yt-dlp. Only sanitized recording metadata is stored in an atomic owner-only cache; audio, coordinates, analytics, and provider credentials are absent. A failed sound service leaves the slideshow available.
...
- Global Controls remappings accept only integer input identifiers for the six known navigation actions. Built-in controls remain reserved and active; mappings cannot contain commands, scripts, paths, URLs, or module-specific behavior.
- The separately deployed diagnostics Worker must use dedicated rate-limit and fingerprint-index KV namespaces, independently sanitize the payload, accept only bundle identifier `com.240mp.jellyfin`, and obtain a short-lived GitHub App installation token. GitHub credentials must never be bundled in OwlSwitch. Without both KV and both Durable Object bindings the Worker fails closed.
- The launch check reads the signed Sparkle feed for update information only. Explicit manual checks use Sparkle’s standard user-driven flow. Signed feed and archive verification remain enabled; releases retain Developer ID signing, notarization, bundle/version identity and arm64 gates. The legacy DMG/checksum remains for older installed apps. No settings, authentication, media, queue or playback data is included in update checks.
